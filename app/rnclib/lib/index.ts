...
/**
 * Toma los valores de los índices almacenados en indices.csv
 * @returns jsonArray
 */
export const getIndices = async () => {
  const csv = require('csvtojson')
  const jsonArray = await csv().fromFile('/app/rnclib/data/indices.csv')
  return jsonArray
}
...
export class CalculadoraCapacidad {
  ...
  /**
   * Retorna una fecha. Si hay redeterminaciones devolverá la última fecha, en caso de no existir redeterminaciones, 
   * retornara la fecha de adjudicación.
   * @param obra 
   * @returns 
   */
  getFechaAdjudicacionORedeterminacion = (obra: DDJJObra) => {
    if (obra.redeterminaciones && obra.redeterminaciones.length !== 0) {
      //let fechaRedeterminacion = _.last(obra.redeterminaciones.map(e => moment(e.fecha, 'DD/MM/YYYY').toDate().getTime()).sort());
      let fechaRedeterminacion = _.last(obra.redeterminaciones.map(e => moment(e.fecha, 'DD/MM/YYYY').toDate()).sort());
      console.log('#### Se filtro por fecha Redeterminacion: ', fechaRedeterminacion);      
      return fechaRedeterminacion;
    }

    console.log('#### Se filtro por fecha Adjudicacion: ', obra.datosObra[0].fechaAdjudicacion);
    return obra.datosObra[0].fechaAdjudicacion;
  }

  /**
   * Nuevo cálculo del compromiso. Toma la Fecha de Adjudicación o Redeterminacion
   * @param obra 
   * @returns 
   */
  getCompromiso(obra: DDJJObra) {
    ...
    const indiceInflacion = this.getIndice(moment(this.getFechaAdjudicacionORedeterminacion(obra), 'DD/MM/YYYY').format('MYYYY'))
    console.log('#### Indice de Inflacion: ', indiceInflacion);
