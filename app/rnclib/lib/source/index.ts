...
export class CalculadoraCapacidad {
  ...
  getFechaAdjudicacionORedeterminacion = (obra: DDJJObra) => {
    if (obra.redeterminaciones && obra.redeterminaciones.length !== 0) {
      console.log('#### Fecha Redeterminacion: ', _.last(obra.redeterminaciones.map(e => moment(e.fecha, 'DD/MM/YYYY').toDate().getTime()).sort()));
      return _.last(obra.redeterminaciones.map(e => moment(e.fecha, 'DD/MM/YYYY').toDate().getTime()).sort());
    }

    console.log('#### fecha Adjudicacion: ', obra.datosObra[0].fechaAdjudicacion);
  ...
  getCompromiso(obra: DDJJObra) {
    ...
    console.log('#### indiceInflacion: ', indiceInflacion);
